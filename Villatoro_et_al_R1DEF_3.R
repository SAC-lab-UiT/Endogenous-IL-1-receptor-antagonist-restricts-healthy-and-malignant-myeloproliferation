...
#we want to remove raw 32 event from plot as std.err = Inf and upper/lower = NA
survival.data <- as.data.frame(p$data)
survival.data[32, "surv"] <- survival.data[31, "surv"]
library(ggplot2)


# Create a step plot of survival data for each group
survival.data$time_by_365 <- survival.data$time / 365
survival.data[32, "surv"] <- survival.data[31, "surv"]
ggplot(survival.data, aes(x = time_by_365, y = surv)) +
  geom_step(aes(color = strata)) +
  scale_color_npg(labels = c("High IL1RN expression", "Low IL1RN expression")) + theme(legend.background = element_blank()) +
  theme(legend.direction = "vertical", legend.position = c(0.7,0.9), legend.title = element_blank()) +
  xlab("Time") + ylab("Relapse-free probability")
