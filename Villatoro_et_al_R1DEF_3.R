...
anovaRes_Global <- anova(resNull,res.cox)
p_value_Global <- round(anovaRes_Global[2, "Pr(>|Chi|)"], 3)
	
fit1 <- survfit(Surv(surv, dead) ~ il1rnFactor,
               data = modelFrame[keep,])

p1 <- ggsurvplot(fit1, data = modelFrame[keep,],palette = c('red','blue'))
p1 <- p1+ggtitle('Global AML patients')
p1$plot <- p1$plot + annotate("text", x = 15, y = 0.2, label = paste0("", p_value_Global),
...
anovaRes_M4M5 <- anova(resNull,res.cox)
p_value_M4M5 <- round(anovaRes_M4M5[2, "Pr(>|Chi|)"], 3)
...
p2 <- p2+ggtitle('M4-M5 AML patients only')
p2$plot <- p2$plot + annotate("text", x = 15, y = 0.2, label = paste0("", p_value_M4M5),
                              hjust = 1, vjust = 0, size = 4, color = "black")

myplots_out <- arrange_ggsurvplots(
  list(Global = p1,M4M5 = p2),
  print = FALSE,
  ncol = 1,
  nrow = 2,
  title = "")
...
ggsave(
  myplots_out,
  file = "output/SurvivalGlobal3.pdf",
  width = 5,
  height = 7)
...
dd <- prcomp(t(cnts))
par(xpd=T)
pdf("PCA_Nras", width = 8, height = 8)
plot(dd$x, pch=20, col=as.character(colData(dds)$Color), cex=1)
text(dd$x, rownames(dd$x), cex=0.6, pos=2.8)
dev.off()
...
pdf("Nras_Upregulated.pdf", width = 8, height = 6)
vennDiagram(all_genes, "up", main="Up")
dev.off()
pdf("Nras_Downregulated.pdf", width = 8, height = 6)
...
# Defined cell types
# Previous ImmGen classification by SingleR, with reorder of legend
new_order <- c("Monocytes", "Neutrophils", "NK cells", "Stem cells")
bySample.cca$celltype_cluster_reval_previous <- factor(bySample.cca$celltype_cluster_reval_previous, levels = new_order)

pList[[length(pList)+1]] <- DimPlot(bySample.cca,
                                    reduction = "tsne",
                                    group.by = "celltype_cluster_reval_previous",
                                    label = F, cols = color.list,
                                    split.by = "SampleName")
...
exclude_categories <- c("HSC", "PreDC_II", "CMoP_I")
