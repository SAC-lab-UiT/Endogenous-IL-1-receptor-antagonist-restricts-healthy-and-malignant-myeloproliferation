...
pdf(file = 'VAV_CRE.pdf',width = 3,height = 4) 
...
# manually curate genes to label and create a column "toLabel" in .nfkbAnnot.xlsx files with value =1 for genes to label
...
#class(t$ensembl_transcript_id)="logical"
#pheno <- readxl::read_excel("pheno_Nestin.xlsx")
#pheno <- as.data.table(pheno[!is.na(pheno$ID), ])
#colnames(pheno) <- gsub(" ", "_", colnames(pheno))
#counts <- counts[, match(pheno$files, gsub("X", "", colnames(t)))]
#colnames(counts) <- pheno$`RNA-seq`
...
class(df$ensembl_transcript_id_version)="character"
